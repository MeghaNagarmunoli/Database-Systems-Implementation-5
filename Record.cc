...
void Record :: PrintToString (Schema *mySchema, char *printHere) {
	...
	for (int i = 0; i < n; i++) {
		...
		// print the attribute name
		sprintf(printHere, "%s%s: [", printHere, atts[i].name);
		//cout << atts[i].name << ": ";
		...
		if (atts[i].myType == Int) {
			int *myInt = (int *) &(bits[pointer]);
			//cout << *myInt;	
			sprintf(printHere, "%s%d", printHere, *myInt);

		// then is a double
		} else if (atts[i].myType == Double) {
			double *myDouble = (double *) &(bits[pointer]);
			//cout << *myDouble;	
			sprintf(printHere, "%s%f", printHere, *myDouble);

		// then is a character string
		} else if (atts[i].myType == String) {
			char *myString = (char *) &(bits[pointer]);
			sprintf(printHere, "%s%s", printHere, myString);

			//cout << myString;	
		} 

		sprintf(printHere, "%s]", printHere);
		//cout << "]";
