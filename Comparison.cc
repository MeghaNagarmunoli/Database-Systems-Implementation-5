...
void OrderMaker ::PrintToStream(std::ostream &os, Schema *schema) const
{
	os << "NumAtts = "<<numAtts<<endl;;
	//os<<sch->GetAtts();
	// for (int i = 0; i < numAtts; i++)
	// {
		
	// 	os << i  <<": "<< whichAtts[i];  //("%3d: %5d\n", i,whichAtts[i]);
	// 	if (whichTypes[i] == Int)
	// 		os << "Int\n";
...
void CNF ::PrintToStream(std::ostream& os, Schema *schema,const Record &literal) const
{
	for (int i = 0; i < numAnds; i++)
	{
		os << "(";
		for (int j = 0; j < orLens[i]; j++)
		{
			if (orList[i][j].operand1 == Left || orList[i][j].operand1 == Right) {
				os << schema->GetAtts()[orList[i][j].whichAtt1].name;
			}
			else {
				if (orList[i][j].attType == Int)
					os << (int *) &(literal.bits[((int *) literal.bits)[1]]);
				else if (orList[i][j].attType == Double)
					os << (double *) &(literal.bits[((int *) literal.bits)[1]]);
				else 
					os << (char *) &(literal.bits[((int *) literal.bits)[1]]);
			}

			if (orList[i][j].op == LessThan)
				os << " < ";
			else if (orList[i][j].op == GreaterThan)
				os << " > ";
			else
				os << " = ";

			if (orList[i][j].operand2 == Left || orList[i][j].operand2 == Right) {
				os << schema->GetAtts()[orList[i][j].whichAtt2].name;
			}
			else {
				if (orList[i][j].attType == Int)
					os << *((int *) &(literal.bits[((int *) literal.bits)[1]]));
